...
fn test_snapshot_scanmem_part(scanmem_program: &str, target_pid: u32, nthreads: u32, verbose: bool) -> Result<u64, String> {
    ...
    scanmem_process.write_line_stdin("snapshot").unwrap();
...
pub fn scenario_func_test_snapshot(reference_scanmem_program: &str, test_scanmem_program: &str, synthetic_load_program: &str, synthetic_load_random_seed: u64, fixture_index: usize, verbose: bool) -> TestResult {
    ...
    let reference_res = test_snapshot_scanmem_part(reference_scanmem_program, synthetic_load_process_pid, 0, verbose);
    ...
    for i in 0..THREAD_COUNT_ARRAY.len() {
        ...
        let test_res = test_snapshot_scanmem_part(test_scanmem_program, synthetic_load_process_pid, nthreads, verbose);
        ...
        // Verify.
        expect_eq_r!(test_result, test_match_count, reference_match_count, format!("nthreads {} failed", THREAD_COUNT_ARRAY[i]));
    }
